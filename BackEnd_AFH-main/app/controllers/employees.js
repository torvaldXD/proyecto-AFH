const Employee = require("../models/employees");

module.exports.getAllEmployees = (req, res) => {
  Employee.find({}).exec(function (err, result) {
    try {
      if (err) {
        res.send({ message: "Error generate in DB" });
      }
      res.json({ docs: result });
      return;
    } catch (error) {
      console.log("Error generate in data base");
    }
  });
};

module.exports.getEmployee = (req, res) => {
  Employee.find({ _id: req.params.id }).exec(function (err, result) {
    try {
      if (result[0]) {
        res.send({ success: true, employee: result[0] });
      }
    } catch (err) {
      res.send({ message: "Employee not found" });
    }
  });
};

module.exports.createEmployee = (req, res) => {
  try {
    //search user if exists
    Employee.find({ idNumber: req.body.employee.idNumber }).exec(function (
      err,
      result
    ) {
      if (result[0]) {
        res.status(200).json({
          success: true,
          message: "The user already exists in BD",
        });
      } else {
        //new employee

        const employee = new Employee({
          name: req.body.employee.name,
          lastName: req.body.employee.lastName,
          numberPhone: req.body.employee.numberPhone,
          idNumber: req.body.employee.idNumber,
          completeName: `${req.body.employee.name} ${req.body.employee.lastName}` 
        });
        //save the user create into the DB
        employee.save(function (err, usuarioDB) {
          if (err) {
            console.log(err);
          }
          res.status(201).json({
            success: true,
            message: "Employee create sucessfull",
          });
        });
      }
    });
  } catch (error) {
    //handle errors
    console.log(error);
  }
};

module.exports.updateEmployee = (req, res) => {
  Employee.findOneAndUpdate(
    { _id: req.params.id },
    {
      $set: {
        name: req.body.employee.name,
        lastName: req.body.employee.lastName,
        email: req.body.employee.email,
        numberPhone: req.body.employee.numberPhone,
        completeName: `${req.body.employee.name} ${req.body.employee.lastName}` 
      },
    }
  ).exec(function (err, result) {
    try {
      if (err) {
        res.json({ success: false, message: "Employee not found" });
      } else {
        res
          .status(200)
          .json({ success: true, message: "Employee updated successfully" });
      }
    } catch (error) {
      console.log("Error generate:", error);
    }
  });
};

module.exports.findByAutoComplete = function(req, res) {
  Employee.find({
      $or: [
          { name: { $regex: req.query.filters, $options: "i" } },
          { lastName: { $regex: req.query.filters, $options: "i" } },
      ]
  }).exec(function(err, docs) {
      if (err) {
          console.info('filter query page: ', err);
          return res.json({
              success: false,
              message:"Something wrong in BD",
          });
      }
      res.json({
          success: true,
          docs
      });
  });
};

module.exports.deleteEmployee = function (req, res) {
  Employee.findByIdAndRemove({ _id: req.params.id }).exec(function (err, doc) {
    if (err) {
      res.json({
        success: false,
        message: "something happened in the database",
      });
    } else {
      // console.log(doc);
      // Codes.findOneAndRemove({code:doc.codeTool}).exec()
      console.log("se elimino la herramienta",doc);
      res.status(200).json({
        success: true,
        message: "Employee deleted successfully",
        employee:doc
      });
    }
  });
};

module.exports.findByPage = function(req, res) {
  const filters = JSON.parse(req.query.filters);
  // console.log("employees",filters);
  let desde = req.query.desde || 0;
  desde = Number(desde);
  let limite = req.query.limite || 100;
  limite = Number(limite);

  const query = {};

  if (filters.global && filters.global.value) {
    const regex = new RegExp(filters.global.value, "i");
    query["$or"] = [
      { item: regex },
      { date: regex },
      { typeItem: regex },
      { type: regex }
    ];
  }

  Employee.find(query)
    .skip(desde)
    .limit(limite)
    .exec(function(err, resultBD) {
      if (err) {
        console.info(err);
        return res.json({
          success: false,
          err
        });
      }
      Employee.countDocuments(query, function(err, count) {
        if (err) {
          console.info(err);
        }
        res.json({
          success: true,
          totalResults: count,
          docs: resultBD
        });
      });
    });
};